...
st.set_page_config(
    page_title="( 4friends+2 couples ) Trading Engine", layout="centered"
)
...
st.markdown(
    """
    <div class="hero-card">
        <h2 style='color: #00ffcc; margin: 0 0 5px 0; text-shadow: 0 0 15px rgba(0,255,204,0.8);'>⚡ (4friends+2 couples) TRADING ENGINE ⚡</h2>
        <p style='color: #94a3b8; font-size: 14px; margin: 0;'>Live Feed + S/R Zones + Timeframes (5s to 5m) + Risk Manager</p>
    </div>
""",
    unsafe_allow_html=True,
)
...
# Risk Management & Martingale Staking
st.markdown(
    """
    <div class="risk-card">
        <span style="color: #f59e0b; font-weight: bold; font-size: 15px;">💰 Risk Management & Suggested Stake</span><br>
        <span style="color: #94a3b8; font-size: 13px;">Safe Money Management calculator for next execution step.</span>
    </div>
""",
    unsafe_allow_html=True,
)
...
st.info(
    f"📌 **Suggested Staking Plan:** Step 1: **${step1}** | Step 2 (Martingale 1):"
    f" **${step2}** | Step 3 (Martingale 2): **${step3}**"
)
      ...
      signal_slot.markdown(
          f"""
                <div class="signal-up">
                    ...
                    Expiry: {selected_timeframe} (Next Candle Open)<br>
