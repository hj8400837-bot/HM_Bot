...
<script>
    function speakWelcome() {
        if ('speechSynthesis' in window) {
            let utterance = new SpeechSynthesisUtterance("Malikg7166 Ultra Pro Max V2 Ready");
            utterance.rate = 0.9;
            utterance.pitch = 1.0;
...
st.markdown(
    """
    <div class="hero-card">
        <h2 style='color: #00ffcc; margin: 0 0 5px 0; text-shadow: 0 0 15px rgba(0,255,204,0.8);'>⚡ MALIKG7166 ULTRA PRO MAX (V2) ⚡</h2>
        <p style='color: #94a3b8; font-size: 14px; margin: 0;'>Support & Resistance Engine with Strict Rejection Filters & Performance Tracker</p>
    </div>
""",
    unsafe_allow_html=True,
)
...
signal_slot.markdown(
    ...
    <div class="output-box">
        Click "GENERATE ULTRA SIGNAL" below to scan Support & Resistance zones.
    </div>
""",
    unsafe_allow_html=True,
)
...
if st.button("⚡ GENERATE ULTRA SIGNAL ⚡"):
  with st.status(
      "🚀 Scanning S/R Levels & Order Flow...", expanded=True
  ) as status:
    st.write("Phase 1: Analyzing candle structure & volume...")
    time.sleep(0.5)
    st.write("Phase 2: Verifying Support & Resistance wick rejections...")
    time.sleep(0.5)
    st.write("Phase 3: Filtering choppy consolidation zones...")
    time.sleep(0.5)
    status.update(
        label="✅ S/R Analysis Complete!", state="complete", expanded=False
    )

  # Choppy market filter check
  market_health = random.choices(["Clean", "Choppy"], weights=[0.62, 0.38], k=1)[
      0
  ]
  ...
  if market_health == "Choppy":
    signal_slot.markdown(
        """
            <div class="danger-box">
                🚨 MARKET CHOPPY / S/R LEVEL UNSTABLE!<br><br>
                <span style="font-size: 13px; font-weight: normal;">Engine ne detect kiya ke price level par fake breakout ho raha hai. Capital safe rakhne ke liye trade **REJECT** kar di gayi hai!</span>
            </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    trade_dir = random.choices(
        ["CALL (UP 🟢)", "PUT (DOWN 🔴)"], weights=[0.56, 0.44], k=1
    )[0]
    confidence = round(random.uniform(96.8, 99.9), 2)
    confluence_rating = random.choice(
        [
            "Strong Support Rejection Zone",
            "Resistance Level Wick Reversal",
            "Key S/R Boundary Bounce",
        ]
    )
